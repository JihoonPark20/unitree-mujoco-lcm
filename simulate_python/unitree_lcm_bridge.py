...
class UnitreeSdk2Bridge:
    ...
    def PublishWirelessController(self) -> None:
        ...
        if not axis_id or not button_id:
            return
        key_state = [0] * 16
        key_state[key_map["R1"]] = self.joystick.get_button(button_id["RB"])
        key_state[key_map["L1"]] = self.joystick.get_button(button_id["LB"])
        key_state[key_map["start"]] = self.joystick.get_button(button_id["START"])
        key_state[key_map["select"]] = self.joystick.get_button(button_id["SELECT"])
        key_state[key_map["R2"]] = 1 if self.joystick.get_axis(axis_id["RT"]) > 0 else 0
        key_state[key_map["L2"]] = 1 if self.joystick.get_axis(axis_id["LT"]) > 0 else 0
        key_state[key_map["A"]] = self.joystick.get_button(button_id["A"])
